      ...
      <ul className="divide-y divide-gray-200 dark:divide-gray-800">
        ...
        {!currentFolder && folders.map((folder) => (
          ...
          >
            ...
            <span className="font-medium text-gray-900 dark:text-gray-100">{folder}</span>
          </li>
        ))}

        {/* Render Files */}
        {displayedFiles.map((f: any) => (
          <li key={f.key} className="p-4 flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-800/30 transition">
            ...
            <div className="flex gap-4">
              <button onClick={() => handleDownload(f.key)} className="text-blue-600 hover:underline text-sm font-semibold">Download</button>
              <button onClick={() => handleDelete(f.key)} className="text-red-600 hover:underline text-sm font-semibold">Delete</button>
            </div>
          </li>
        ))}
